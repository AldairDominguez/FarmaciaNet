...
namespace Farmacias
{
    internal class Connections
    {
        ...
        private Forma_X f1;
        private Forma_Consultas f2;
        private Login l1;
        private Ventas V1;
        private ConsultaVentas CV1;
        private GerenteVentas GV1;
        ...
        public void eliminarEmp(string num)
        {
            Singleton.Instance.OpenDBConnection();
            // Eliminar el registro de la tabla usuarios
            command = new SqlCommand(string.Format("DELETE FROM usuarios WHERE idusuario='{0}'", num), Singleton.Instance.GetDBConnection());
            int usuariosEliminados = command.ExecuteNonQuery();

            if (usuariosEliminados == 0)
            {
                MessageBox.Show("No se encontró un usuario que coincida con el id especificado.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }

            // Eliminar el registro de la tabla empleados
            command = new SqlCommand(string.Format("DELETE FROM empleados WHERE idempleado='{0}'", num), Singleton.Instance.GetDBConnection());
            int empleadosEliminados = command.ExecuteNonQuery();

            if (empleadosEliminados == 0)
            {
                MessageBox.Show("No se encontró un empleado que coincida con el id especificado.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }

            MessageBox.Show("El empleado y usuario se han eliminado correctamente.", "Información", MessageBoxButtons.OK, MessageBoxIcon.Information);
        }
        ...
        public void busquedaEdInv(string num)
        {
            ...
            // Verificar si el DataReader contiene algún registro
            if (leer.HasRows)
            {
                leer.Read();
            ...
            else
            {
                ...
                MessageBox.Show("No se encontraron existencias para el producto y el almacén especificados.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }

            leer.Close();
        }

        public void eliminarInv(string num)
        {
            Singleton.Instance.OpenDBConnection();
            command = new SqlCommand(string.Format("DELETE FROM inventario WHERE inventario.idproducto='{0}' AND inventario.idalmacen='{1}'", num, 101), Singleton.Instance.GetDBConnection());
            ...
            {
                MessageBox.Show("No se encontró un registro que coincida con los criterios de búsqueda especificados.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        public void busquedaEdEmp(string num)
        {
            ...
            {
                // Mostrar un mensaje de error si el registro no existe
                MessageBox.Show("El empleado con el id especificado no existe.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                leer.Close();
                return;
            }
            ...
            {
                // Mostrar un mensaje de error si el registro no existe
                MessageBox.Show("El usuario con el id especificado no existe.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        ...
        {
            Singleton.Instance.OpenDBConnection();
            command = new SqlCommand(string.Format("SELECT Nombre, Responsable, Direccion, RFC, Telefono, Correo FROM proveedor p WHERE p.idproveedor='{0}'", numprov), Singleton.Instance.GetDBConnection());
            leer = command.ExecuteReader();
            ...
            {
                // Mostrar un mensaje de error si el registro no existe
                MessageBox.Show("El proveedor con el id especificado no existe.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }

            leer.Close();
        }

        public void ElimonarEdProv(string numprov)
        {
            Singleton.Instance.OpenDBConnection();

            // Verificar si el proveedor existe antes de eliminarlo
            command = new SqlCommand(string.Format("SELECT COUNT(*) FROM proveedor WHERE idproveedor='{0}'", numprov), Singleton.Instance.GetDBConnection());
            ...
            command = new SqlCommand(string.Format("SELECT Nombre, Responsable, Direccion, RFC, Telefono, Correo FROM proveedor WHERE idproveedor='{0}'", numprov), Singleton.Instance.GetDBConnection());
            leer = command.ExecuteReader();
            leer.Read();
            f1.tbxEdPNom.Text = leer.GetString(0);
            f1.tbxEdPResp.Text = leer.GetString(1);
            ...
            // Mostrar un cuadro de diálogo de confirmación antes de eliminar el registro
            DialogResult dialogResult = MessageBox.Show("¿Está seguro de que desea eliminar este proveedor?", "Confirmar eliminación", MessageBoxButtons.YesNo);
            if (dialogResult == DialogResult.Yes)
            {
                // Eliminar el registro de proveedor con el id seleccionado
                command = new SqlCommand(string.Format("DELETE FROM proveedor WHERE idproveedor='{0}'", numprov), Singleton.Instance.GetDBConnection());
                command.ExecuteNonQuery();
            }
        }
        ...
        {
            Singleton.Instance.OpenDBConnection();
            command = new SqlCommand(string.Format("SELECT producto, Activo, idvia, reqReceta, Precio, idproveedor FROM productos p WHERE p.idproducto='{0}'", num), Singleton.Instance.GetDBConnection());
            leer = command.ExecuteReader();
            ...
            {
                // Mostrar un mensaje de error si el registro no existe
                MessageBox.Show("El producto con el id especificado no existe.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }

            leer.Close();
        }

        public void eliminarProd(string num)
        {
            Singleton.Instance.OpenDBConnection();

            // Eliminar el registro de la tabla productos
            command = new SqlCommand(string.Format("DELETE FROM productos WHERE idproducto='{0}'", num), Singleton.Instance.GetDBConnection());
            int productosEliminados = command.ExecuteNonQuery();

            if (productosEliminados == 0)
            {
                MessageBox.Show("No se encontró un producto que coincida con el id especificado.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
        ...
        public void ActualizarTxt(string id, string almacen)
        {
            ...
            + "and inventario.idproducto=productos.idproducto and inventario.idproducto={0} and almacen.idalmacen={1}", int.Parse(id), int.Parse(almacen)), Singleton.Instance.GetDBConnection());
            ...
            {
                //aqui que se abra una forma en donde debera capturar los datos de la receta no aqui no  con el message box basta
                V1.txtNP.Text = reader.GetSqlValue(1).ToString();
                V1.lblP.Text = reader.GetSqlValue(2).ToString();
                V1.txtCP.Focus();
...
"where empleados.idempleado=factura.idempleado and empleados.idsucursal=sucursal.idsucursal and empleados.idempleado={0}", vendedor), Singleton.Instance.GetDBConnection());
        ...
        #endregion VentasGerente

        public void receta(string nombreDoctor, string cedula, string idFactura, string apellido)
        {
            try
            {
                Singleton.Instance.OpenDBConnection();
                SqlCommand AgregarF = new SqlCommand("INSERT INTO receta (idreceta, nombreDoctor, Cedula, idfactura, Apellido) VALUES (@idreceta, @nombreDoctor, @cedula, @idFactura, @apellido)", Singleton.Instance.GetDBConnection());
                int idReceta = new Random().Next();
                AgregarF.Parameters.AddWithValue("@idreceta", idReceta);
                AgregarF.Parameters.AddWithValue("@nombreDoctor", nombreDoctor);
                AgregarF.Parameters.AddWithValue("@cedula", cedula);
                AgregarF.Parameters.AddWithValue("@idFactura", idFactura);
                AgregarF.Parameters.AddWithValue("@apellido", apellido);
                AgregarF.ExecuteNonQuery();
            }
            catch (Exception ex)
            {
                MessageBox.Show("Error al insertar datos en la tabla receta: " + ex.Message);
            }
        }
    }
}